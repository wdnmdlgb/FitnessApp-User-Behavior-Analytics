    ...
    (function() {{
        ...
        function updateCharts(d) {{
            ...
            charts.premium.setOption({{
                ...
                series: [
                    {{ name: '免费用户', type: 'bar', data: premiumData.map(x => x.type === '免费用户' ? [x.session_duration, x.workouts, x.calories, x.ai_usage*100, x.plan_usage*100] : null)[0], itemStyle: {{ color: '#85CDCA' }} }},
                    {{ name: '付费用户', type: 'bar', data: premiumData.map(x => x.type === '付费用户' ? [x.session_duration, x.workouts, x.calories, x.ai_usage*100, x.plan_usage*100] : null)[0], itemStyle: {{ color: '#2c5282' }} }}
                ]
            }});
